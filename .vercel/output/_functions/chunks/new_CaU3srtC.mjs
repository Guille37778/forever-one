...
const $$New = createComponent(async ($$result, $$props, $$slots) => {
  ...
  if (Astro2.request.method === "POST") {
    ...
    if (!name || !price) {
      ...
      const { data: product, error: productError } = await supabase.from("products").insert({ name, description, price, collection_id: collectionId, category, subcategory_id: subcategoryId, image_urls: imageUrls }).select().single();
      if (productError) {
